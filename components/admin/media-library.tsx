...
function useUnsavedChangesGuard(hasUnsavedChanges: boolean) {
  ...
  useEffect(() => {
    const message = "คุณมีการแก้ไข Album หรือ Tag ที่ยังไม่ได้บันทึก ต้องการออกจากหน้านี้หรือไม่?";
    ...
    const handleLinkClick = (event: MouseEvent) => {
      if (!unsavedChangesRef.current || event.defaultPrevented || event.button !== 0 || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) return;
...
function MediaCollectionManagement({ assets, albums, tags, onDirty }: { assets: OrganizedMediaAsset[]; albums: AdminMediaAlbum[]; tags: AdminMediaTag[]; onDirty: () => void }) {
  ...
  const confirmAlbumDelete = (name: string) => window.confirm(`ยืนยันการลบ Album “${name}” ใช่หรือไม่? รูปจริงจะไม่ถูกลบ`);
  const confirmTagDelete = (name: string) => window.confirm(`ยืนยันการลบ Tag #${name} ใช่หรือไม่? รูปจริงจะไม่ถูกลบ`);
  return <details className="admin-media-collection-management"><summary>จัดการ Album และ Tag ที่สร้างแล้ว</summary><p>แก้ชื่อหรือคำอธิบายได้ที่นี่ การลบกลุ่มจะนำรูปออกจากกลุ่มเท่านั้น ไม่ลบไฟล์รูปจริงหรือการใช้งานรูปบนเว็บไซต์</p><div className="admin-media-collection-management__lists"><section><h3>Album ({albums.length})</h3>{albums.length === 0 ? <span>ยังไม่มี Album</span> : albums.map((album) => { const updateAction = updateMediaAlbum.bind(null, album.id); const deleteAction = deleteMediaAlbum.bind(null, album.id); return <details key={album.id} className="admin-media-collection-management__item"><summary>{album.name} · {albumCount(album.id)} รูป</summary><form action={updateAction} onChange={onDirty}><label>ชื่อ Album<input name="name" defaultValue={album.name} maxLength={120} required /></label><label>คำอธิบาย (ไม่บังคับ)<input name="description" defaultValue={album.description} maxLength={1000} /></label><div className="admin-media-collection-management__actions"><FormSubmitButton label="บันทึก Album" pendingLabel="กำลังบันทึก…" /></div></form><form action={deleteAction} onSubmit={(event) => { if (!confirmAlbumDelete(album.name)) event.preventDefault(); }}><FormSubmitButton className="admin-text-button" label="ลบ Album นี้" pendingLabel="กำลังลบ…" /></form></details>; })}</section><section><h3>Tag ({tags.length})</h3>{tags.length === 0 ? <span>ยังไม่มี Tag</span> : tags.map((tag) => { const updateAction = updateMediaTag.bind(null, tag.id); const deleteAction = deleteMediaTag.bind(null, tag.id); return <details key={tag.id} className="admin-media-collection-management__item"><summary>#{tag.name} · {tagCount(tag.id)} รูป</summary><form action={updateAction} onChange={onDirty}><label>ชื่อ Tag<input name="name" defaultValue={tag.name} maxLength={60} required /></label><div className="admin-media-collection-management__actions"><FormSubmitButton label="บันทึก Tag" pendingLabel="กำลังบันทึก…" /></div></form><form action={deleteAction} onSubmit={(event) => { if (!confirmTagDelete(tag.name)) event.preventDefault(); }}><FormSubmitButton className="admin-text-button" label="ลบ Tag นี้" pendingLabel="กำลังลบ…" /></form></details>; })}</section></div></details>;
}
...
export function MediaLibrary({ assets, albums, tags, organizationAvailable }: { assets: OrganizedMediaAsset[]; albums: AdminMediaAlbum[]; tags: AdminMediaTag[]; organizationAvailable: boolean }) {
  ...
  useUnsavedChangesGuard(hasUnsavedCollectionChanges);
