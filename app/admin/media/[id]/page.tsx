...
export default async function EditMediaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <><header className="admin-page-header"><p className="eyebrow">Edit image</p><h1>Media details</h1><p>Keep the alt text descriptive and set the focal point for responsive cover crops.</p></header><div className="admin-media-preview"><Image className="admin-media-preview__crop" style={{ objectPosition: `${asset.focalX}% ${asset.focalY}%` }} src={asset.publicUrl} alt={asset.altText} width={1040} height={840} /><a href={asset.publicUrl} target="_blank" rel="noreferrer">Open full-size image</a></div><form className="admin-editor" action={updateAction}><label className="admin-editor__wide">Alt text<input name="altText" defaultValue={asset.altText} required /></label><label className="admin-editor__wide">Caption (optional)<textarea name="caption" rows={4} defaultValue={asset.caption} /></label><div className="admin-editor__wide"><strong>Focal point for responsive Covers</strong><FocalPointPicker imageUrl={asset.publicUrl} alt={asset.altText} initialX={asset.focalX} initialY={asset.focalY} /></div><div className="admin-editor__wide admin-form-actions"><button className="button" type="submit">Save changes</button><Link href="/admin/media">Cancel</Link></div></form><section className="admin-danger"><h2>Delete image</h2><p>This removes the image metadata and file. Images that are still in a gallery cannot be deleted until removed from that gallery.</p><form action={deleteAction}><button className="button button--danger" type="submit">Delete permanently</button></form></section></>;
}
