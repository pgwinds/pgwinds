...
import { getAdminGallery, getAdminGalleryImages, getAdminMediaPickerAssets } from "@/lib/queries/admin-content";
...
export default async function EditGalleryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [gallery, images, mediaAssets] = await Promise.all([getAdminGallery(id), getAdminGalleryImages(id), getAdminMediaPickerAssets()]);
  if (!gallery) notFound();
