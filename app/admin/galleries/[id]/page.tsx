...
import { addAlbumToGallery, addImageToGallery, deleteGallery, removeImageFromGallery, updateGallery } from "@/lib/actions/admin";
import { getAdminGallery, getAdminGalleryImages, getAdminMediaLibrary } from "@/lib/queries/admin-content";
...
export default async function EditGalleryPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ image?: string }> }) {
  const { id } = await params;
  const [gallery, images, library, query] = await Promise.all([getAdminGallery(id), getAdminGalleryImages(id), getAdminMediaLibrary(), searchParams]);
  if (!gallery) notFound();
